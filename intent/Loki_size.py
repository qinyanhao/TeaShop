...
def getResult(inputSTR, utterance, args, resultDICT):
    ...
    if utterance == "[一杯][大]冰[綠][半糖][少冰]":
        if args[1] in userDefinedDICT["size"]:
            resultDICT["size"].append(sizeDICT[args[1]])
        else:
            pass

    if utterance == "[原鄉][兩杯]，[一杯][半糖][少冰]，[一杯][全糖][正常冰]":
        resultDICT["size"].append('大杯')
        

    if utterance == "[嚴選高山茶][兩分][糖][大]杯[少冰]":
        if args[3] in userDefinedDICT["size"]:
            resultDICT["size"].append(sizeDICT[args[3]])
        else:
            pass
    ...
    if utterance == "[嚴選高山茶][微糖][大]杯[少冰]":
        if args[2] in userDefinedDICT["size"]:
            resultDICT["size"].append(sizeDICT[args[2]])
        else:
            pass
